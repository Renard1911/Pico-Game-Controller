...
void ws2812b_trail(uint32_t counter, bool hid_mode) {
  ...
  for (int i = 0; i < NUM_TRAIL_POINTS; i++) {
    ...
    while (trail_positions[i] < 0) trail_positions[i] += WS2812B_LED_SIZE;
    while (trail_positions[i] >= WS2812B_LED_SIZE) trail_positions[i] -= WS2812B_LED_SIZE;
  }
  
  // Decay all trail brightness values
  for (int i = 0; i < WS2812B_LED_SIZE; i++) {
    if (trail_brightness[i] > 0) {
      trail_brightness[i] = trail_brightness[i] > 8 ? trail_brightness[i] - 8 : 0;
    }
  }
  ...
  for (int point = 0; point < NUM_TRAIL_POINTS; point++) {
    ...
    if (pos >= 0 && pos < WS2812B_LED_SIZE) {
      ...
      if (trail_brightness[prev_pos] < 180) trail_brightness[prev_pos] = 180;
      if (trail_brightness[next_pos] < 180) trail_brightness[next_pos] = 180;
    }
  }
